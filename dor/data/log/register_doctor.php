...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "val_usuario") {
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE sru_nombre_usuario = '$nombre_usuario' AND sru_modulo = 'med' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;

        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(adm_usr_id) FROM web_users WHERE username = '$nombre_usuario' AND adm_usr_tipo = 'med' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
    ...
    if ($strTipoValidacion == "val_mail") {
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE sru_email = '$email' AND sru_modulo = 'med' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;

        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(adm_usr_id) FROM web_users WHERE mail = '$email' AND adm_usr_tipo = 'med' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuario = isset($idRow) ? $idRow  : 0;
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE UPPER(sru_dpi) = UPPER('$dpi') AND sru_modulo = 'med' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;
        ...
        <tr border="1">
            ...
            <b>Hemos recibido una solicitud para la creacion de un usuario en el modulo de medicos de nuestra
...
<form id="formData" method="POST">
    ...
    <div class="form-group row">
        <label for="DPI" class="col-sm-6 col-form-label">DPI</label>
        <div class="col-sm-6">
            <input type="text" name="dpi" id="dpi" class="form-control" required onblur="fntValDpi()"><span class="highlight"></span><span class="bar"></span>
        </div>
    </div>
    <div class="form-group row">
        <label for="Nombres" class="col-sm-6 col-form-label">Nombres</label>
        <div class="col-sm-6">
            <input type="text" name="nombres" id="nombres" class="form-control" required><span class="highlight"></span><span class="bar"></span>
        </div>
    </div>
    <div class="form-group row">
        <label for="Apellidos" class="col-sm-6 col-form-label">Apellidos</label>
        <div class="col-sm-6">
            <input type="text" name="apellidos" id="apellidos" class="form-control" required><span class="highlight"></span><span class="bar"></span>
        </div>
    </div>
    <div class="form-group row">
        <label for="No. de Colegiado" class="col-sm-6 col-form-label"> No. de Colegiado</label>
        <div class="col-sm-6">
            <input type="text" name="colegiado" id="colegiado" class="form-control" required><span class="highlight"></span><span class="bar"></span>
        </div>
    </div>
    <div class="form-group row">
...
<script>
    ...
    function fntInsertUs() {
        ...
        valorNom = document.getElementById("dpi").value;
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese un numero de dpi');
            return false;
        }

        valorNom = document.getElementById("nombres").value;
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese un nombre nombre');
            return false;
        }

        valorNom = document.getElementById("apellidos").value;
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese un apellido');
            return false;
        }

        valorNom = document.getElementById("colegiado").value;
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese un numero de  colegiado');
            return false;
        }
        ...
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese un nombre de usuario');
            return false;
        }
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "register_doctor.php?ajax=true&validaciones=insert_form_user",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
                        //$('#formData')[0].reset();
                        ...
                        $('#formData')[0].reset();
                        window.location = "login_doctor.php?usr=6";
                    }
                } else {
    ...
    function fntValUsuario() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "register_doctor.php?ajax=true&validaciones=val_usuario",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
    ...
    function fntValDpi() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "register_doctor.php?ajax=true&validaciones=val_dpi",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {

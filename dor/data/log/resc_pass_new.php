...
$mail = trim($_GET["ml"]) ? $_GET["ml"]  : '';
$mail = openssl_decrypt($mail, COD, KEY);

$var_consulta = pg_query($rmfAdm, "SELECT COUNT(adm_usr_id) FROM web_users WHERE UPPER(mail) = UPPER('$mail') AND proces_new_pass = 1 LIMIT 1;");
if ($row = pg_fetch_array($var_consulta)) {
    $idRow = trim($row[0]);
}
$proces = isset($idRow) ? $idRow  : 0;

if ($proces == 0) {
    session_unset();
    session_destroy();
    header("Location:../../urlUsage.html");
    exit();
}

$CTG_DT_ = trim($_GET["dt"]) ? $_GET["dt"]  : '';
$CTG_DT_ = openssl_decrypt($CTG_DT_, COD, KEY);
...
if (isset($_REQUEST['clave1'])) {
    ...
    $nomUs = trim($_GET["usn"]) ? $_GET["usn"]  : '';
    $nomUs = openssl_decrypt($mail, COD, KEY);
...
<hgroup>
    ...
    <form name="registration" id="registration" action="" method="post">
        ...
        <button type="submit" name="submit" value="Register" class="button buttonBlue">Enviar

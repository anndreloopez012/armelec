...
if ($variableUsr >= 1) {
    ...
    if (isset($_POST['username'])) {
        $rs = pg_query($rmfAdm, "SELECT COUNT(sru_id) from sol_regis_user WHERE sru_nombre_usuario ='$username' and sru_modulo ='$usuario' ");
        if ($row = pg_fetch_array($rs)) {
            $idRow = $row[0];
        }
        $variableStatus = isset($idRow) ? $idRow  : 0;
    }

    if ($variableStatus >= 1) {
        header('Location:login_pacientes.php?error=5');
    } else {
        if (isset($_POST['username'])) {
                ...
                if (isset($_POST['username'])) {
                    ...
                    if ($rows == 1) {
                        ...
                        if ($contador >= 5) {
                            ...
                            header('Location:login_pacientes.php?error=2');
                        } else {
                            ...
                            if ($validar) {
                                ...
                                header('Location:login_pacientes.php?error=1');
                            }
                        }
                    }
                }
            } else {
                header('Location:login_pacientes.php?error=4');
            }
        }
    }
}
...
<form action="" method="post" name="login">
    ...
    <div id="register-link" class="text-right">
        <a href="resc_pass.php?us=pac" class="text-info">Olvidaste tu Contraseña?</a>
    </div><br>
    ...
    <div id="register-link" class="text-right">
        <?php
        if ($RegPac == 1) { ?>
            <a href="register_paciente.php" class="text-info">Crear usuario nuevo!</a>
        <?php
        } ?>
    </div><br>
    <div style="font-size:15px; text-align: center;color: #9B9B9B;">
    ¿Quiéres tener acceso y control sobre todo tu historial médico en un solo lugar? Contáctanos al correo a <a href="mailto:gut.contacto@visualmed.online" title="Abre el administrador de correo electronico haciendo clic aqui">gut.contacto@visualmed.online</a> y con gusto te atenderemos.
    </div>
</form>

...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "val_usuario") {
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE sru_nombre_usuario = '$nombre_usuario' AND sru_modulo = 'pub' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;

        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(adm_usr_id) FROM web_users WHERE username = '$nombre_usuario' AND adm_usr_tipo = 'pub' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
    ...
    if ($strTipoValidacion == "val_mail") {
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE sru_email = '$email' AND sru_modulo = 'pub' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;

        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(adm_usr_id) FROM web_users WHERE mail = '$email' AND adm_usr_tipo = 'pub' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuario = isset($idRow) ? $idRow  : 0;
        ...
        $val = 1;
        if ($contrato >= 1) {
            $var_consulta = pg_query($rmfAdm, "SELECT COUNT(ctg_con_id) FROM ctg_contratos WHERE ctg_con_id = '$contrato' AND ctg_con_tpo = '12' LIMIT 1;");
            if ($row = pg_fetch_array($var_consulta)) {
                $idRow = trim($row[0]);
            }
        }
        ...
        $arrData = array();
        $var_consulta = "SELECT * FROM ctg_contratos WHERE ctg_con_id = '$contrato' AND ctg_con_tpo = '12' LIMIT 1;";
...
<?PHP
	...
	<tr border="1">
		<td align="center" bgcolor= "#0464fc">
            ...
            <tr border="1">
            <td align="center"><b>AGENCIA DE PUBLICIDAD' . $nombre_comercial . '</b></td><br>
            </tr>
            </table><br><br>
            ...
            <b>Hemos recibido una solicitud para la creacion de un usuario en el modulo de publicidad de nuestra
    ...
    <script>
        ...
        function fntInsertUs() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "register_publicidad.php?ajax=true&validaciones=insert_form_user",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            //$('#formData')[0].reset();
                            ...
                            $('#formData')[0].reset();
                            window.location = "login_publicidad.php?usr=6";
        ...
        function fntValUsuario() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "register_publicidad.php?ajax=true&validaciones=val_usuario",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
        ...
        function fntValContratoSolReg() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "register_publicidad.php?ajax=true&validaciones=val_contrato_sol_reg",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            document.getElementById("loading-screen").style.display = "none";
        ...
        function fntValContratoWebUser() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "register_publicidad.php?ajax=true&validaciones=val_contrato_web_user",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            document.getElementById("loading-screen").style.display = "none";
        ...
        function fntValContrato() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "register_publicidad.php?ajax=true&validaciones=val_contrato",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
        ...
        function fntDataContrato() {
            ...
            $.ajax({
                ...
                url: "register_publicidad.php?ajax=true&validaciones=data_contrato",
                data: datos,
                async: true,
                global: false,
                type: "post",

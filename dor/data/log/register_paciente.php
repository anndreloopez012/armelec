...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "val_usuario") {
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE sru_nombre_usuario = '$nombre_usuario' AND sru_modulo = 'pac' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;

        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(adm_usr_id) FROM web_users WHERE username = '$nombre_usuario' AND adm_usr_tipo = 'pac' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
    ...
    if ($strTipoValidacion == "val_mail") {
        ...
        $val = 1;
        $var_consulta = pg_query($rmfAdm, "SELECT COUNT(sru_id) FROM sol_regis_user WHERE UPPER(sru_dpi) = UPPER('$dpi') AND sru_modulo = 'pac' LIMIT 1;");
        if ($row = pg_fetch_array($var_consulta)) {
            $idRow = trim($row[0]);
        }
        $usuarioCode = isset($idRow) ? $idRow  : 0;
        ...
        <tr border="1">
            ...
            <b>Hemos recibido una solicitud para la creacion de un usuario en el modulo de pacientes de nuestra
...
<script>
    ...
    function fntInsertUs() {
        ...
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese un nombre email');
            return false;
        }

        valorNom = document.getElementById("nombre_usuario").value;
        if (valorNom == null || valorNom.length == 0 || /^\s+$/.test(valorNom)) {
            alertify.alert('AVISO ', 'Por favor seleccione o ingrese una nombre de usario');
            return false;
        }
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "register_paciente.php?ajax=true&validaciones=insert_form_user",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
                        //$('#formData')[0].reset();
                        ...
                        window.location = "login_pacientes.php?usr=6";
                    }
                } else {
    ...
    function fntValUsuario() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "register_paciente.php?ajax=true&validaciones=val_usuario",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
    ...
    function fntValDpi() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "register_paciente.php?ajax=true&validaciones=val_dpi",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {

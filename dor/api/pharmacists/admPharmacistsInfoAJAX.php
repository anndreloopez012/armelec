<script>
    ...
    function fntUpdate_1() {

        valor = document.getElementById("ctg_far_hortda1").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:Horario de atencion');
            return false;
        }

        valor = document.getElementById("ctg_far_hortda2").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:Horario de atencion');
            return false;
        }

        valor = document.getElementById("ctg_far_serdom1").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:Horario de atencion');
            return false;
        }

        valor = document.getElementById("ctg_far_serdom2").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:Horario de atencion');
            return false;
        }

        valor = document.getElementById("ctg_far_dir").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:direccion');
            return false;
        }

        valor = document.getElementById("ctg_far_zona").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:zona');
            return false;
        }

        valor = document.getElementById("ctg_far_tels").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:telefono');
            return false;
        }

        valor = document.getElementById("ctg_far_enc_dpi").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:dpi');
            return false;
        }

        valor = document.getElementById("ctg_far_enc_nombre").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:nombre');
            return false;
        }

        valor = document.getElementById("sexo_").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
        ...
        alertify.confirm('Tu Perfil', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "pharmacistsInfo.php?ajax=true&validaciones=update_1",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
    ...
    function fntUpdate_0() {
        ...
        alertify.confirm('Tu Perfil', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "pharmacistsInfo.php?ajax=true&validaciones=update_0",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            alertify.alert('Aviso!', 'Datos cargados correctamente');
    ...
    function fntValidacioncodigo() {
        ...
        if (!isNaN(intcolegiado) && (intcolegiado > 0)) {
            ...
            $.ajax({
                ...
                url: "pharmacistsInfo.php?validaciones=validacion_colegiado&colegiado=" + intcolegiado,
                async: true,
                global: false,
    ...
    function fntDibujoDropDep() {
        ...
        $.ajax({
            ...
            url: "pharmacistsInfo.php?validaciones=dibujo_dropdow_dep",
            data: {},
            async: true,
            global: false,
            type: "post",
    ...
    function fntDibujoDropMun() {
        ...
        $.ajax({
            ...
            url: "pharmacistsInfo.php?validaciones=dibujo_dropdow_mun",
            data: {
                region: strReg,
            },
            async: true,
        ...
        $('#form-perfil').on('change', 'input[type="file"]', function(event) {
            ...
            if (file_data.name) {
                ...
                if ($('#' + id).hasClass('tbl-btn-upload') || extensionesPermitidas.indexOf(strExtension) !== -1) {
                    ...
                    $.ajax({
                        url: "pharmacistsInfo.php?validaciones=proces_img",
                        dataType: 'json',
                        async: true,
                        cache: false,
                        contentType: false,

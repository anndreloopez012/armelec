<script>
    ...
    function fntUpdate() {
        ...
        valor = document.getElementById("ctg_hos_enc_nom1").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
        ...
        alertify.confirm('Tu Perfil', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "hospitalInfo.php?ajax=true&validaciones=update",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                        ...
                        alertify.alert('Aviso', 'Debe llenar todos los datos del formulario!');
                        //location.reload(); 
                    }
                }
            });
    ...
    function fntValidacioncodigo() {
        ...
        if (!isNaN(intcolegiado) && (intcolegiado > 0)) {
            ...
            $.ajax({
                ...
                url: "hospitalInfo.php?validaciones=validacion_colegiado&colegiado=" + intcolegiado,
                async: true,
                global: false,
    ...
    function fntDibujoDropDep() {
        ...
        $.ajax({
            ...
            url: "hospitalInfo.php?validaciones=dibujo_dropdow_dep",
            data: {},
            async: true,
            global: false,
            type: "post",
    ...
    function fntDibujoDropMun() {
        ...
        $.ajax({
            ...
            url: "hospitalInfo.php?validaciones=dibujo_dropdow_mun",
            data: {
                region: strReg,
            },
            async: true,
        ...
        $('#form-perfil').on('change', 'input[type="file"]', function(event) {
            ...
            if (file_data.name) {
                ...
                if ($('#' + id).hasClass('tbl-btn-upload') || extensionesPermitidas.indexOf(strExtension) !== -1) {
                    ...
                    $.ajax({
                        url: "hospitalInfo.php?validaciones=proces_img",
                        dataType: 'json',
                        async: true,
                        cache: false,
                        contentType: false,

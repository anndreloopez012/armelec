<script>
    ...
    function fntInsertNew() {
        ...
        alertify.confirm('Tus Productos', 'Seguro que desea continuar?  ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "hospitalProducts.php?ajax=true&validaciones=insert_new",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
                            $('#Tabla').load('hospitalProducts.php?validaciones=busqueda_table');
                            $('#TablaNuevo').load('hospitalProducts.php?validaciones=busqueda_table_new');
    ...
    function fntInsert() {
        ...
        valor = document.getElementById("ctg_hpp_code").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:codigo');
            return false;
        }

        valor = document.getElementById("ctg_hpp_descrip").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Aviso', 'Por favor complete el siguiente campo:descripcion');
            return false;
        }

        valor = document.getElementById("ctg_hpp_pre").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
        ...
        alertify.confirm('Tus Productos', 'Seguro que desea continuar?  ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "hospitalProducts.php?ajax=true&validaciones=insert",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntUpdatePrecio() {
        ...
        alertify.confirm('Tus Productos', 'Seguro que desea continuar?  ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "hospitalProducts.php?ajax=true&validaciones=update_precio",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntDelete() {
        ...
        alertify.confirm('Tus Productos', 'Seguro que desea continuar?  ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "hospitalProducts.php?ajax=true&validaciones=delete",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntValidacioncodigo() {
        ...
        if (!isNaN(intnombre) && (intcnombre > 0)) {
            ...
            $.ajax({
                ...
                url: "hospitalProducts.php?validaciones=validacion_nombre&nombre=" + intnombre,
                async: true,
                global: false,
    ...
    function fntDibujoTabla() {
        ...
        $.ajax({
            ...
            url: "hospitalProducts.php?validaciones=busqueda_table",
            data: {
    ...
    function fntDibujoTablaNew() {
        ...
        $.ajax({
            ...
            url: "hospitalProducts.php?validaciones=busqueda_table_new",
            data: {
                Search: strSearch,
            },
            async: true,
    ...
    function fntValidacionCode() {
        ...
        var intCode = $("#ctg_hpp_code").val();

            $.ajax({

                url: "hospitalProducts.php?ajax=true&validaciones=validacion_code&code=" + intCode,
                async: true,
                global: false,

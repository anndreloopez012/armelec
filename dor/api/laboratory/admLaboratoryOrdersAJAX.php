<script>
    ...
    function fntInsert() {
        ...
        alertify.confirm('Tus Pedidos', 'Seguro que desea continuar?  ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "laboratoryOrders.php?ajax=true&validaciones=insert",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
                            $('#Tabla').load('laboratoryOrders.php?validaciones=busqueda_table');
    ...
    function fntUpdate() {
        ...
        alertify.confirm('Tus Pedidos', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "laboratoryOrders.php?ajax=true&validaciones=update",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#Tabla').load('laboratoryOrders.php?validaciones=busqueda_table');
    ...
    function fntValidacioncodigo() {
        ...
        if (!isNaN(intnombre) && (intcnombre > 0)) {
            ...
            $.ajax({
                ...
                url: "laboratoryOrders.php?validaciones=validacion_nombre&nombre=" + intnombre,
                async: true,
                global: false,
    ...
    function fntDibujoTabla() {
        ...
        $.ajax({
            ...
            url: "laboratoryOrders.php?validaciones=busqueda_table",
            data: {

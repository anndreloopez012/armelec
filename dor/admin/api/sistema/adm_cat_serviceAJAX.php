<script>
    ...
    function fntInsert() {
        ...
        valor = document.getElementById("ctg_ser_code").value;
        ...
        valor = document.getElementById("ctg_ser_descrip").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Tus Pacientes', 'Por favor complete el siguiente campo:Nombres');
            return false;
        }
        ...
        valor = document.getElementById("ctg_ser_obs").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
            alertify.alert('Tus Pacientes', 'Por favor complete el siguiente campo:Nombres');
            return false;
        }
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "cat_service.php?ajax=true&validaciones=proces",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntEdit() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "cat_service.php?ajax=true&validaciones=edit",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            fntDibujoTabla()
    ...
    function fntDelete() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "cat_service.php?ajax=true&validaciones=delete",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            fntDibujoTabla()
    ...
    function fntDibujoTabla() {
        ...
        $.ajax({
            ...
            url: "cat_service.php?validaciones=busqueda_tabla",
            data: {
                Search: $strSearch,
            },
            async: true,
    ...
    function fntValCodigo() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "cat_service.php?ajax=true&validaciones=val_codigo",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {

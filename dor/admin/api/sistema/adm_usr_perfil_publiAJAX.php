<script>
    ...
    function fntInsert() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "usr_perfil_publi.php?ajax=true&validaciones=proces",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntDelete() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "usr_perfil_publi.php?ajax=true&validaciones=delete",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            fntDibujoTabla()
    ...
    function fntDibujoTabla() {
        ...
        $.ajax({
            ...
            url: "usr_perfil_publi.php?validaciones=busqueda_tabla",
            data: {
                Search: $strSearch,
            },
            async: true,
    ...
    function fntTablaContrato() {

        var $strSearch = $("#SearchContrato").val();

        //alert(strCategori + "                                  strCategori");

        $.ajax({

            url: "usr_perfil_publi.php?validaciones=tabla_contrato",
            data: {
                Search: $strSearch,
            },
            async: true,
    ...
    function fntValUsuario() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "usr_perfil_publi.php?ajax=true&validaciones=val_usuario",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
    ...
    function fntValMail() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "usr_perfil_publi.php?ajax=true&validaciones=val_mail",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
                        alertify.alert('AVISO', 'Esta email ya existe!');
                        document.getElementById("ctg_pub_email").value = "";
                    }
                } else {
                    if (r.status == 0) {
                        alertify.set('notifier', 'position', 'bottom-right');
    ...
    function fntValEmpresa() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "usr_perfil_publi.php?ajax=true&validaciones=val_com_empresa",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
    ...
    function fntValSucEmpresa() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "usr_perfil_publi.php?ajax=true&validaciones=val_suc_empresa",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
                        alertify.alert('AVISO', 'Esta empresa ya existe!');
    ...
    function fntValContrato() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "usr_perfil_publi.php?ajax=true&validaciones=val_contrato",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
    ...
    function fntDibujoDropDep() {
        ...
        $.ajax({
            ...
            url: "usr_perfil_publi.php?validaciones=dibujo_dropdow_dep",
            data: {},
            async: true,
            global: false,
            type: "post",
    ...
    function fntDibujoDropMun() {
        ...
        $.ajax({
            ...
            url: "usr_perfil_publi.php?validaciones=dibujo_dropdow_mun",
            data: {
                region: strReg,
            },
            async: true,

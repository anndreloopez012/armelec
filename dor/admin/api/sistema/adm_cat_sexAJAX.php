<script>
    ...
    function fntInsert() {
        ...
        valor = document.getElementById("ctg_sex_cod").value;
        ...
        valor = document.getElementById("ctg_sex_desc").value;
        if (valor == null || valor.length == 0 || /^\s+$/.test(valor)) {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "cat_sex.php?ajax=true&validaciones=proces",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntEdit() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "cat_sex.php?ajax=true&validaciones=edit",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            fntDibujoTabla()
    ...
    function fntDelete() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "cat_sex.php?ajax=true&validaciones=delete",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            fntDibujoTabla()
    ...
    function fntDibujoTabla() {
        ...
        $.ajax({
            ...
            url: "cat_sex.php?validaciones=busqueda_tabla",
            data: {
                Search: $strSearch,
            },
            async: true,
    ...
    function fntValCodigo() {
        ...
        $.ajax({
            ...
            data: datos,
            dataType: 'json',
            url: "cat_sex.php?ajax=true&validaciones=val_codigo",
            success: function(r) {
                if (r.status) {
                    if (r.status == 1) {
